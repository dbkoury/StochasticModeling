...
Since we are using predictive analysis, where we take data from previous experience and judgement and use them to solve a new current problem, there are bound to some pitfalls which would potentially decrease the accuracy of our analysis. 
Firstly, if we are using the numbers that include finding the probability to receive bonuses or forecasting client's savings, large amount of relevant data is required to get a good result. And a lot of times, big data sets are hard to come by. Secondly, we have to condider that the expected value of both Option 2 and 3 pull are derived based on assumptions of probabilities. 
Option 2: If pursued, there is a `{(1-PBonus)*100}`% chance of ending up with a net loss of `{r round(Op2EVRev)}` dollars. 
Option 3: If pursued, there is a massive dependency on the team's consensus estimate of Appshop's chances of winning the RFP at the $150,000 bid with `{PWin*100}`%. The `{PWin*100}`% probability of winning the bid in itself is less favorable than a fair coin toss which makes this option suboptimal for a risk-averse client.
We have to consider the above mentioned weightage of assumed probabilities in our derivation of expected values when choosing our potential option, because although the overall expected value of the options seem profitable, it would distort our expected profits because the results are derived on the basis of assumptions and it ignores the impact of the inevitable variations. Additionaly, it also brings along a level of risk which may be undersirable. 
One must also note that there are other variables that were not covered or were not known to Appshop, such as the true client savings, a new potential competitor who can outbid Appshop, etc. It is imperative to look at the pitfalls of the analysis as Appshop is taking significant risks, such as having the team and company go through so many work hours with the possibility of a loss (Option 2 with no bonus). In addition to company's resources spent on the OS-7 project, Clark's compensation and workers' hourly revenue depends on the financial performance in this project.
